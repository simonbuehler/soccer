...
export const useAppStore = defineStore("app", () => {
  ...
  const allPlayers = computed(() => [...fieldPlayers.value, ...benchPlayers.value]);
  ...
  function movePlayerToField(playerId: string, x: number, y: number): boolean {
    ...
    // Finde und entferne den Spieler von der Bank
    ...
    console.log(`[AppStore] Player ${player.firstName} moved to field at (${x}, ${y})`);
    return true;
  }
